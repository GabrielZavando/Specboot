# Capability: specboot-json-standard

Freeze the canonical `.specboot.json` schema (required + optional fields, including opt-in `layers`) and ship a deterministic validation script (`validate-specboot.sh`) plus documentation, so every Specboot project and Specboot itself can declare and validate its configuration before linting or updating.
