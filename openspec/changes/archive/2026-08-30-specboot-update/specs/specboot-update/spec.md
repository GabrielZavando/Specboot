...
`specboot update` is the supported upgrade path for an already-initialized project. It replaces
the framework's intocable files "without mercy" (opción A, `docs/framework-contract.md` §"Modelo
de actualización"), warns on a major version jump, never touches the developer's `docs/` or
project code, and rewrites `frameworkVersion` in `.specboot.json`. It also doubles as a repair
tool for hand-corrupted intocable files (`docs/docs-standard.md` rows B/C/D).
