# Capability: versioning-standard

Establish the canonical SemVer versioning standard and breaking-change matrix for the `@gabrielzavando/specboot` framework, so that every `specboot update` communicates whether the consuming project must act (major) or can update transparently (minor/patch).
