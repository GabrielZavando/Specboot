...
- [ ] 1.1 Escribir el test TDD primero (RED): `tests/workflow-isolation-test.sh`
...
- [ ] 1.2 Enmendar `tests/package-files-test.sh` (RED): el allowlist npm excluye
  ...
  plantilla).
- [ ] 1.3 Crear `templates/github/workflows/consumer-ci.yml` — el workflow CI
...
- [ ] 1.4 Crear `templates/github/workflows/deploy.example.yml` — plantilla
  ...
  `DEPLOY_ENABLED`).
- [ ] 1.5 Crear `templates/github/pull_request_template.md` (fuente canónica del
  PR template que `init` instala en `.github/pull_request_template.md`).
- [ ] 1.6 Actualizar `package.json#files`: remover `.github/workflows`, incluir
  `templates/github/` y la fuente del PR template; verificar que `release.yml`
  quede fuera del paquete.
- [ ] 1.7 Actualizar `specboot.sh` (FRAMEWORK_ITEMS y lógica de init): reemplazar
...
- [ ] 2.1 Escribir los tests TDD primero (RED) con fixtures consumidores: update
...
- [ ] 2.2 Implementar en `specboot update` la detección del `release.yml`
...
- [ ] 2.3 Implementar la advertencia y exigencia de resolución explícita cuando
  el `release.yml` heredado fue modificado (no eliminar automáticamente).
- [ ] 2.4 Garantizar que los workflows personalizados y ajenos al framework
...
- [ ] 3.1 Escribir el test TDD primero (RED): `tests/apply-preflight-test.sh` —
...
- [ ] 3.2 Reescribir el pre-flight de `.opencode/commands/apply.md`: ejecutarlo
  ...
  fail-closed).
- [ ] 3.3 Explicitar la suciedad permitida: primera ejecución limitada a
...
- [ ] 3.4 Explicitar la interacción con la checklist `## Mandatory Steps` (su
...
- [ ] 4.1 Escribir los tests TDD primero (RED): extender
...
- [ ] 4.2 Extender `docs/agent-permission-contracts.yml` con el contrato
...
- [ ] 4.3 Actualizar los front matter de `.opencode/agents/*.md`: `build`
...
- [ ] 4.4 Actualizar la documentación de los agentes afectados para que coincida
...
- [ ] 5.1 Escribir los tests TDD primero (RED): fixtures de
  ...
  de GitHub CLI).
- [ ] 5.2 Implementar la lectura de `opencode.json` del proyecto validado (vía
...
- [ ] 5.3 Corregir la semántica de patrones: `*` como comodín multi-carácter y
  `?` como comodín de un carácter conforme a OpenCode (hoy `?` se escapa como
  literal).
- [ ] 5.4 Comparar el `mode` real del front matter con el manifiesto y validar
  `permission.task` (contrato de la Tarea 4).
- [ ] 5.5 Fiscalizar independientemente `can_commit`, `can_push`,
  `can_manage_prs`, `can_run_arbitrary_code` y `can_spawn_subagents` (sin derivar
  push/PR desde `can_commit`).
- [ ] 5.6 Implementar la detección de bypasses: variantes de Git (force-push en
...
- [ ] 5.7 Preferir allowlists de Git de solo lectura para agentes implementadores
...
Suggested Path: `scripts/validate-agent-permissions.mjs`, `docs/agent-permission-contracts.yml`
...
- [ ] 6.1 Escribir los tests TDD primero (RED): fixtures consumidores — SC-012,
...
- [ ] 6.2 Corregir `specboot.sh`: los modos `--ci`/`--init` validan el directorio
...
- [ ] 6.3 Alinear `check_permission_contracts()` con la resolución corregida: el
...
- [ ] 7.1 Escribir el test TDD primero (RED): `tests/command-contracts-test.sh` —
...
- [ ] 7.2 Implementar la validación desde el front matter (parser real, no
...
- [ ] 7.3 Verificar que el contrato válido pasa y un fixture corrupto falla
...
- [ ] 8.1 Test RED: aserciones en los tests de regresión — `archive.md` sin la
...
- [ ] 8.2 Corregir `.opencode/commands/archive.md` (description sin "staging") y
  ...
  `{file:...}`.
- [ ] 8.3 Actualizar `ai-specs/skills/plan-change/SKILL.md`: generar `## Why`
...
- [ ] 8.4 Corregir `README.md` para distinguir workflows internos del
...
- [ ] 8.5 Mover `check_permission_contracts()` fuera de `check_ci_cd()` en
  `specboot.sh` (eliminando la dependencia implícita de orden), manteniendo su
  comportamiento y sección visible.
- [ ] 8.6 Verificación final SC-014: `bash tests/run-all.sh`, `bash
